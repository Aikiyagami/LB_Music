...
async def es_img(_, message):
    ...
    try:
        ...
        if image:
            sent_message = await message.reply_photo(image, caption="ʙʏ [ɴɪᴄᴏ ʀᴏʙɪɴ](https://t.me/NicoRobinXRobot)", parse_mode=None)
            asyncio.create_task(delete_message_after_timeout(sent_message, 10))  # Delete after 1 minute
        else:
